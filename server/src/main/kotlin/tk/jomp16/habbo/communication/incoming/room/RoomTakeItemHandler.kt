...
class RoomTakeItemHandler {
    ...
    fun handle(habboSession: HabboSession, habboRequest: HabboRequest) {
        ...
        if (habboSession.currentRoom!!.removeItem(roomItem)) {
            val ownerHabboSession = HabboServer.habboSessionManager.getHabboSessionById(roomItem.userId) ?: return

            ownerHabboSession.habboInventory.addItems(listOf(UserItem(
                    roomItem.id,
                    roomItem.userId,
                    roomItem.baseName,
                    roomItem.extraData,
