...
class UserClothingsResponse {
    ...
    fun response(habboResponse: HabboResponse, clothings: Collection<String>) {
        habboResponse.apply {
            val figureSetIds = HabboServer.habboGame.itemManager.furniXMLInfos.filterKeys { clothings.contains(it) }.flatMap {
                it.value.customParams.split(',').map(String::trim).map(String::toInt)
            }
