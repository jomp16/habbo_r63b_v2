...
package tk.jomp16.fastfood.communication.incoming.user
...
class UserCreditsHandler {
    @FFHandler(FFIncoming.GET_USER_CREDITS)
    fun handle(fastFoodSession: FastFoodSession, habboRequest: HabboRequest) {
        if (!fastFoodSession.authenticated) return

        fastFoodSession.sendHabboResponse(FFOutgoing.USER_CREDITS, fastFoodSession.habboSession!!.userInformation.credits)
    }
}